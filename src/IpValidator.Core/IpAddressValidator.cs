...
public class IpAddressValidator : IIpAddressValidator
{
    ...
    {
        if (!IsValidIPv4(ip))
            throw new ArgumentException("Некорректный формат IP.");

        string[] segments = ip.Split('.');
        return new byte[] {
            byte.Parse(segments[0]),
            byte.Parse(segments[1]),
            byte.Parse(segments[2]),
            byte.Parse(segments[3])
        };
    }
}